...
# realsense driver with 2 cameras for RGB and point cloud 
...
class RealSenseRGBAndPcPublisher(Node):
    def __init__(self):
        super().__init__('PC_AND_RGB_PUBLISHER')
    ...
    def timer_callback(self):
        self.publish_camera_data1(self.pipeline1, self.rgb_publisher1, "camera1")
        self.publish_camera_data1(self.pipeline2, self.rgb_publisher2, "camera2")

        self.publish_camera_data2(self.pipeline1, self.pc_publisher1, "camera1")
        self.publish_camera_data2(self.pipeline2, self.pc_publisher2, "camera2")


    def publish_camera_data1(self, pipeline, rgb_publisher, frame_id):
        try:
    ...
    def publish_camera_data2(self, pipeline, pc_publisher, frame_id):
        try:
            frames = pipeline.wait_for_frames(timeout_ms=500)
...
def main(args=None):
    ...
    node = RealSenseRGBAndPcPublisher()
    try:
        rclpy.spin(node)
