...
class DeviceModel:
    ...
    def openDevice(self):
        ...
        try:
            ...
            self.isOpen = True
            print("{} open".format(self.serialConfig.portName))
            ...
            t.start()
            print("\nDevice has been successfully switched on")
        except SerialException:
            print("display of " + self.serialConfig.portName + " failed")

    # 监听串口数据线程 Listening to serial data threads
    def readDataTh(self, threadName, delay):
        print("activate " + threadName)
        while True:
            # 如果串口打开了
            if self.isOpen:
                try:
    ...
    def get_readBytes(self, devid, regAddr, regCount):
        ...
        # print("tempBytes: ", tempBytes) # --> works fine
    ...
    def loopRead(self):
        print("Loop read start") # after this error appears
        while self.loop: # whats the stop condition?
